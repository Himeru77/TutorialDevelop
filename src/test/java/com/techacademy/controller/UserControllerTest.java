...
class UserControllerTest {
    ...
    void test_getList() throws Exception{
        ...
        assertEquals(user.get(0).getName(),"キラメキ太郎");
        assertEquals(user.get(1).getId(),2);
        assertEquals(user.get(1).getName(),"キラメキ次郎");
        assertEquals(user.get(2).getId(),3);
        assertEquals(user.get(2).getName(),"キラメキ花子");
    }
